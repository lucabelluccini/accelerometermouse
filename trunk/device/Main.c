...
/*********************************************************************
/* Define STDMOUSEHID if you want to enable standard Mouse HID
/* Comment it out if you want to enable Vendor Defined HID
*********************************************************************/
//#define STDMOUSEHID
...
#define RIGHT_MB_CLICKS					3
#define EVENT_WINDOW_ms					650
#define EVENT_WINDOW_INT_ticks			-EVENT_WINDOW_ms*47				// 1.398 s whole INT cycle -> 65536 / 1398 = 47 units equals 0.001 ms

#pragma udata
BOOL rightMB;						// Right Mouse Button Flag
char buffer[3];
USB_HANDLE lastTransmission;
UINT16 X, Y, oldX, oldY;			// X and Y data to be sent
...
/** VECTOR REMAPPING ***********************************************/
	...
	void MouseLowPriorityISRCode()
	{
		...
		if (INTCONbits.TMR0IF)
		{
			...
			INTCONbits.TMR0IF = 0;
			//Re-starts Timer
			WriteTimer0(EVENT_WINDOW_INT_ticks);
			if(clickCount >= RIGHT_MB_CLICKS){
				mLed_Toggle();
				rightMB = TRUE;	
			}
			clickCount = 0;
...
void MouseInit(void)
{
	...
	// Initialize click count in Event Window, starts Timed Interrupt
	clickCount = 0;
	EventWindowInit();
	WriteTimer0(EVENT_WINDOW_INT_ticks);
...
{   
    ...
    if(HIDTxHandleBusy(lastTransmission) == 0)
    {
        ...
        // Mouse Buttons - Virtual Right Mouse Button
		if(rightMB)
		{
			buffer[0] |= 0x02;
			rightMB = FALSE;
		...
		// Write back over HID Report
        hid_report_in[0] = buffer[0];
		
		AccelerometerRead(&X,&Y);
		if(oldX < X)
			buffer[1] = +1;
		else
			buffer[1] = -1;
...
#ifndef STDMOUSEHID		
		hid_report_in[3] = 0xf0;
		hid_report_in[4] = 0x0f;
		hid_report_in[5] = 0xe0;
		hid_report_in[6] = 0x0e;
#endif
...
#else
        lastTransmission = HIDTxPacket(HID_EP, (BYTE*)hid_report_in, 0x07);
#endif
    }
